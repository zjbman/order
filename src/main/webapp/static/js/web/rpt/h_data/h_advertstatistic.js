$(function() {
	...
	$('#table').bootstrapTable({   
		url:rootPath+'/rpt/h_data/h_advertstatistic/AdvertList.html',
		method: 'post',   //请求方式（*）
        dataType: "json",
...
//            {field:'appid',title:'appid',sortable:true,visible:false},
	       {field:'datetime',title:'日期',sortable:true,formatter: function(value,row,index){{
            ...
            {field:'advertisername',title:'广告主',sortable:true},
...
function creatHighcharts(data){
	...
	for( var i = 0; i < data.rows.length; i++){
		if(temp6==data.rows[data.rows.length-i-1].datetime){
			sum6+=data.rows[data.rows.length-i-1].totalclickcount;
			if(i==data.rows.length-1){
	...
	var temp7=data.rows[data.rows.length-1].datetime;
	var sum7=0;
	var downloadbegincount=new Array();//开始下载
	for( var i = 0; i < data.rows.length; i++){
		if(temp7==data.rows[data.rows.length-i-1].datetime){
			sum7+=data.rows[data.rows.length-i-1].downloadbegincount;
			if(i==data.rows.length-1){
				downloadbegincount.push(sum7);
			}
		}
		else{
			downloadbegincount.push(sum7);
			temp7 =data.rows[data.rows.length-i-1].datetime;
			sum7=data.rows[data.rows.length-i-1].downloadbegincount;
			if(i==data.rows.length-1){
				downloadbegincount.push(sum7);
				break;
	...
	var temp8=data.rows[data.rows.length-1].datetime;
	var sum8=0;
	var downloadendcount=new Array();//下载完成
	for( var i = 0; i < data.rows.length; i++){
		if(temp8==data.rows[data.rows.length-i-1].datetime){
			sum8+=data.rows[data.rows.length-i-1].downloadendcount;
			if(i==data.rows.length-1){
				downloadendcount.push(sum8);
			}
		}
		else{
			downloadendcount.push(sum8);
			temp8 =data.rows[data.rows.length-i-1].datetime;
			sum8=data.rows[data.rows.length-i-1].downloadendcount;
			if(i==data.rows.length-1){
				downloadendcount.push(sum8);
				break;
	...
	var temp9=data.rows[data.rows.length-1].datetime;
	var sum9=0;
	var installcount=new Array();//安装
	for( var i = 0; i < data.rows.length; i++){
		if(temp9==data.rows[data.rows.length-i-1].datetime){
			sum9+=data.rows[data.rows.length-i-1].installcount;
			if(i==data.rows.length-1){
				installcount.push(sum9);
			}
		}
		else{
			installcount.push(sum9);
			temp9 =data.rows[data.rows.length-i-1].datetime;
			sum9=data.rows[data.rows.length-i-1].downloadendcount;
			if(i==data.rows.length-1){
				installcount.push(sum9);
				break;
			}
	...
	series: [{
	    ...
	    { name: '开始下载',
			 data: downloadbegincount,
			 visible: false,
			 yAxis: 5,       
		},
	    { name: '下载完成',
			 data: downloadendcount,
			 visible: false,
			 yAxis: 6,       
		 },
	    { name: '安装数',
