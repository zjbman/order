$(function() {
	...
	$('#table').bootstrapTable({   
		url:rootPath+'/rpt/app/statistic/AppDetailList.html',
		method: 'post',   //请求方式（*）
        dataType: "json",
	    ...
	    columns:[[
           /* {field:'ck',checkbox:true },*/
	        ...
	        {field:'requestcount',title:'请求数',width:'5%',sortable:true,footerFormatter: function (rows) {
	        		return sumformat(rows,'requestcount');
	        	}},	       
	        {field:'sendcount',title:'发送数',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'sendcount');
        	}},
	        {field:'showcount',title:'展示数',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'showcount');
        	}},
            {field:'clickcount',title:'点击数',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'clickcount');
        	}},
	        {field:'toplaycount',title:'开始播放',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'toplaycount');
        	}},
            {field:'toendcount',title:'结束播放',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'toendcount');
        	}},
            {field:'loadpageclick',title:'落地页点击',width:'7%',sortable:true,footerFormatter: function (rows) {
        		return sumformat(rows,'loadpageclick');
        	}},
	        {field:'totalincome',title:'收入',width:'5%',sortable:true,formatter: function(value,row,index){
                return (value*1).toFixed(2);},footerFormatter: function (rows) {
	        		return sumformat(rows,'totalincome');
	        	}
	        },   
	        {field:'ecpm',title:'ecpm',width:'5%',sortable:true,formatter: function(value,row,index){
