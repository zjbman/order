$(function() { 
	...
	$('#table').bootstrapTable({   
		url:rootPath+'/sys/department/List.html',
		method: 'post',   //请求方式（*）
	    dataType: "json",
	    ...
	    columns:[[    
	        ...
	        {field:'name',title:'部门名称',sortable:true,},   
...
function Winedit(name,id){
	...
	{	
		 $.ajax({
   		  url:rootPath+'/sys/department/Find.html',
   		  type:"post",
   		  data:{"id":id},
   		  dataType:"json",
   		  success: function(data){
...
function save(){
	 ...
	 $.ajax({
		  url:rootPath+'/sys/department/Save.html',
		  type:"post",
...
function del(id){
	$.ajax({
		  url:rootPath+'/sys/department/Delete.html',
 		  type:"post",
 		  data:{"id":id},
 		  dataType:"json",
 		 success: function(data){
